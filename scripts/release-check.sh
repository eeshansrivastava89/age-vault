...
Manual checks:
  [ ] Run `node bin/age-vault.mjs --help` — shows help
  [ ] Run `node bin/age-vault.mjs --version` — shows version
  [ ] Run `echo "test" > /tmp/t.txt && node bin/age-vault.mjs -e /tmp/t.txt` — encrypts
  [ ] Run `node bin/age-vault.mjs -d /tmp/t.txt.age` — decrypts
  [ ] Run `node bin/age-vault.mjs -l /tmp --status` — shows status
